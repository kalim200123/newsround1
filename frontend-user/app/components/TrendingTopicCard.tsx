...
export default function TrendingTopicCard({ topic, rank, displayMode, onClick, className }: TrendingTopicCardProps) {
  ...
  return (
    <button
      ...
      className={cn(
        "group relative w-full overflow-hidden transition-all duration-500 rounded-xl",
        "hover:shadow-xl hover:scale-[1.02]", // Added scale back for Grid usage if desired, or can be overridden
        "outline-none focus-visible:ring-2 focus-visible:ring-ring text-left",
    ...
    >
      ...
      <div className="relative h-full flex flex-col p-3 text-white z-10">
            ...
            {isTopThree ? (
              <div
                className={cn(
                  ...
                  medalColors[rank - 1]?.bg || medalColors[0].bg,
                  medalColors[rank - 1]?.border || medalColors[0].border
                )}
              >
                <Trophy size={16} className={medalColors[rank - 1]?.icon || medalColors[0].icon} />
              </div>
            ) : (
        ...
        <div className="flex-1 mb-2 w-full">
          <p className={cn("font-black leading-tight line-clamp-2 drop-shadow-md", isTopThree ? "text-sm" : "text-xs")}>
            {topic.display_name}
          </p>
        ...
        {/* Stats Row */}
        <div className="flex items-center gap-2 text-[10px] text-white/80 mb-2 mt-auto">
